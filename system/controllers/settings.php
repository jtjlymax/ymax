...
switch ($action) {
    case 'app':
        ...
        if (!empty(_get('testWa'))) {
            $result = Message::sendWhatsapp(_get('testWa'), 'YMaxICS Test Whatsapp');
            r2(U . "settings/app", 's', 'Test Whatsapp has been send<br>Result: ' . $result);
        }
        if (!empty(_get('testSms'))) {
            $result = Message::sendSMS(_get('testSms'), 'YMaxICS Test SMS');
            r2(U . "settings/app", 's', 'Test SMS has been send<br>Result: ' . $result);
        }
        if (!empty(_get('testEmail'))) {
            Message::sendEmail(_get('testEmail'), 'YMaxICS Test Mail', 'YMaxICS Test Email Body');
            r2(U . "settings/app", 's', 'Test Email has been send');
        }
        if (!empty(_get('testTg'))) {
            $result = Message::sendTelegram('YMaxICS Test Telegram');
    ...
    case 'dbbackup':
        ...
        header('Content-Disposition: attachment;filename="YMaxICS_' . count($tables) . '_tables_' . date('Y-m-d_H_i') . '.json"');
